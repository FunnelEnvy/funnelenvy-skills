...
- The per-tactical-hypothesis `slot_available` / `slot_note` routing determination and the run-level `live_program_unreconciled` flag from Phase 3.5 (`phases/validate.md` Gate 7), consumed by the Step 5b live-program fold routing and caveat, and the Step 7 same-surface sequencing note. These are separate from the six-gate record and are NOT inputs to the Step 4d cap arithmetic.
...
**Live-program fold routing (`slot_available: fail`, tactical only).** Route to "What's Not Here" any tactical hypothesis whose `phases/validate.md` Gate 7 returned `slot_available: fail` -- a running or queued experiment already owns the candidate's surface and lever class, or a fold decision covering that surface + lever is on record. This is a **hard fold**: the candidate is removed from the tactical tiers and NOT scored (occupancy is not an ICE question). It is **method-agnostic**: it fires under any `testing_method`, because owning a slot is not an A/B-power concern. Frame the "What's Not Here" entry distinctly from low-traffic infeasibility -- it is an occupied slot, not a traffic problem:

1. Name the live experiment in natural language (no experiment identifiers or system internals, per the Deliverable Purity Constraint).
2. Phrase the recommendation as an iteration *inside* that experiment -- re-sync with it and read it cleanly -- NOT as a standalone re-open.
3. Point to any variation copy or evidence the folded candidate developed, so that work is carried into the running experiment rather than lost.

Take the wording from `slot_note`. A recorded-fold `fail` reads the same way (fold into the experiment the decision named); the regeneration must not silently reverse a fold on record. This routing supersedes any competing minting of the same (surface, lever): a folded candidate never also appears as a scored experiment.

**Live-program-not-reconciled caveat (KB mode).** When the run-level `live_program_unreconciled` flag is set (no experiment-history / program-state source bound; `phases/validate.md` Gate 7 `not-assessed`), surface a visible "live program not reconciled" caveat in the roadmap (rendered via the SKILL.md "What's Not Here" caveat form), so the un-reconciled program is explicit rather than latent. This composes with the existing "Connect a completed-experiment knowledge base" Prerequisite; it is KB-mode-only, so a legacy run (no live-program concept) is unaffected.
...
Live-program one-test-per-surface constraints are part of this layer too: when `phases/validate.md` Gate 7 kept a tactical hypothesis with a same-surface sequencing note (`slot_available: pass` because a running experiment holds the surface with a *different* lever, or an ambiguous-status caution), sequence that hypothesis after the live experiment concludes and render the reason in the Sequencing Rationale prose ("a live experiment currently runs on this surface; this test is sequenced to start after it reads out, so the two do not overlap on the same surface"). Take the wording from `slot_note`. This is a live-program concurrency constraint, distinct from the low-traffic single-concurrent-test prioritization in `construct.md` Step 5b (that one keys on traffic; this one keys on a real running experiment).
