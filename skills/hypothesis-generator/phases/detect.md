...
- Supplementary scope silver (KB mode only, optional): any in-scope `silver-reference-doc` not matched by an enumerated read-side role (e.g., a measurement / lead-lifecycle reference), loaded by the orchestrator per SKILL.md `Read-side Mapping` > `Supplementary Silver (Non-Enumerated) Consumption Contract`. Consumed like `engagement-constraints`: it feeds framing, constraints, and guardrails only (measurement reality, lead-quality / disqualification definitions, down-funnel attribution limits). It NEVER produces opportunities, never enters pattern matching, never touches ICE, and never lifts the no-baseline Confidence cap (it can tighten a guardrail, never loosen). Absent in most runs; skipped with no penalty when absent.
...
In KB mode the orchestrator supplies the same context bodies, sourced from the scope's silver artifacts per the SKILL.md `KB Mode (Dual-Mode Output)` > `Read-side Mapping`. Trigger and detection logic below is source-agnostic and unchanged, with two KB-native additions (no legacy equivalent): the structural observation artifact, consumed by Step 1 and Step 1e only; and supplementary scope silver (non-enumerated `silver-reference-doc`s), consumed as constraints/guardrails alongside the `engagement-constraints` input (Step 1d) and never as an opportunity or trigger source. Both degrade penalty-free when absent.
...
| Experiment-history input (not bound, KB mode -- no external producer AND no in-KB `schema: experiment-history` artifact) | Skip experiment-history triggers (Step 1g), including the live-program state read (`live_program.program_state_readable: false`). NO confidence penalty and NO global cap: absence means no prior-experiment evidence was available. Add "Connect a completed-experiment knowledge base" to Prerequisites, and (KB mode) surface the "live program not reconciled" caveat via `phases/score.md`, so the un-reconciled program is visible rather than latent. |
...
**Accepted schema-version range (the consumer side of the cross-skill contract in `schemas/performance-profile.md` > `Cross-Skill Schema-Version Contract`):** this skill accepts stamped `schema_version` `"2.0"` through `"2.3"` and an absent stamp. ga4-audit stamps `"2.3"`, aa-audit stamps `"2.1"`; both ride the stamp along into the KB-mode `silver-performance-analysis` artifact, so version gating works in both modes. AA and GA4 profiles are interchangeable at a given stamp for every field the stamp guarantees; producer-specific substitutions self-gate per structure below, never by producer detection.
...
**Displacement / inverted-proxy read (net-new annotation).** Beyond minting continuation opportunities, read the bound experiment-history source for a **displacement / inverted-proxy mechanism** on any surface: an on-page proxy (engagement rate, bounce, scroll, dwell) that moved opposite to, or stayed flat against, a downstream business metric that rose (the "downstream-conversion-displacement" class). This is a net-new read: the minting logic above reads only `next_experiments` / `surfaces` / `priority` / `outcome` and never inspects a mechanism or recurring-pattern signal. It produces one carried-forward internal annotation, `displacement_surfaces`, consumed by `construct.md` Step 5.

- **Signal source, both source shapes.** Read the discovered-mechanism / recurring-pattern signal from whichever bound source is in play, tolerating both shapes the way the minting logic already spans them: the external producer `gold-experiment-index` plus the silver insight records it links, and the in-output-KB `schema: experiment-history` silver artifact whose frontmatter carries `mechanisms.discovered` / `recurring_patterns`. Read by **content equivalence**, mirroring the Step 1c `Profile Schema Equivalence` tolerance: whichever field names a displacement / inverted-proxy mechanism class satisfies the read, whether it is a machine token or prose in the insight / learning narrative.
- **Normalize the mechanism class** the same way this step normalizes `parent_outcome` (map the source prose or tokens to a settled class). The class of interest is downstream-conversion-displacement / inverted-proxy: an on-page proxy that moved opposite to, or stayed flat against, a downstream business metric that rose.
- **Derive the observed surfaces from the exhibiting records, not from a frontmatter surface field.** The mechanism frontmatter names the class, not where it was seen. Associate the displacement class with the `Page` / `source_surfaces` of the history records that actually exhibit it (the displacement-outcome records). `displacement_surfaces` carries `{mechanism_class, observed_surfaces}`.
- **Tri-state and penalty-free.** A present displacement signal fires the annotation; an absent signal emits no annotation and applies no confidence penalty and no global cap, exactly like every other experiment-history leg (the penalty-free skips above, the Graceful Degradation experiment-history row). A run whose history carries no displacement mechanism behaves byte-identically to today.
- **Analysis-only.** `displacement_surfaces` is an internal annotation carried forward like the Step 1f annotations; it is never written to disk and never rendered (see SKILL.md `Deliverable Purity Constraint`).

**Output:** Experiment-history-derived opportunities added to the opportunity list, tagged `type: "experiment-history-derived"`; plus, when a displacement / inverted-proxy mechanism is present, the `displacement_surfaces` annotation carried forward to `construct.md` Step 5.

**Live-program state read (net-new annotation).** Beyond minting continuations and reading the displacement mechanism, read the bound experiment-history source for the **current program state**: which experiments are running or queued (occupying their surface and lever) and which fold / consolidation decisions are on record. The minting logic above reads `next_experiments` / `surfaces` / `priority` / `outcome`; the displacement read reads a mechanism signal; this read is distinct again -- it reads the *live status* of the program's experiments so a tactical candidate is never minted for a slot a running experiment already owns. It produces one carried-forward internal annotation, `live_program`, consumed by `phases/validate.md` (the `slot_available` gate). This is the leg-2 read: the reader surfaces program *state*, not only completed-experiment continuation fuel.

- **Both source shapes, by content equivalence.** Read from whichever bound experiment-history source is in play, tolerating both shapes exactly as the minting and displacement legs do: the external producer `gold-experiment-index` plus the silver insight records it links, and the in-output-KB `schema: experiment-history` silver artifact. Read the status by content equivalence, mirroring the Step 1c `Profile Schema Equivalence` tolerance: whichever field or prose carries the experiment's live status satisfies the read, whether a machine `Status` token or a "currently running" / "in flight" / "live on {page}" narrative line.
- **`occupied_slots` (running / queued experiments).** For each record whose read is running or queued -- per the **Outcome normalization table** `Status` column in SKILL.md `Read-side Mapping`: `in-progress`, `monitoring` (still being watched), or a queued / scheduled / committed-to-run `idea` -- emit one entry `{surface, lever_class, status, name, evidence_pointer}`. `surface` is the record's `Page` / `source_surfaces`; `lever_class` is the coarse mechanism class the experiment tests (hero/headline messaging, proof/trust, form, CTA, nav/IA, layout, pricing, personalization), normalized the same way this step normalizes `parent_outcome` (map the record's design prose or tokens to a settled lever class); `name` is a natural-language reference to the experiment; `evidence_pointer` points at any read / variation detail the record carries. A record whose read is **concluded** (a settled `Outcome` with a read-complete status) does NOT occupy its slot: a concluded experiment frees the surface + lever for a legitimate successor (that successor is the normal continuation-minting and evidence-citation path above).
- **`recorded_folds` (documented fold decisions).** Scan the bound source for a documented fold / consolidation decision -- an idea folded into another experiment, or a "run standalone -> rejected, consolidated into {experiment}" note (in a reconciliation record, a per-experiment idea file, or an index annotation). Read by content equivalence. Emit `{surface, lever_class, folded_into, source}`. A recorded fold decision is binding regardless of the named experiment's current status (it is a decision on record, not a status inference), which is what prevents a regeneration from silently reversing a fold.
- **`program_state_readable`.** Set `false` when no experiment-history source binds at all (the same absence that skips the minting and displacement legs). It drives the KB-mode "live program not reconciled" caveat in `phases/score.md`; it is never a confidence penalty.
- **Tri-state and penalty-free.** A present program state fires the annotation; an absent source emits `live_program` with `program_state_readable: false` and no `occupied_slots` / `recorded_folds`, applying no confidence penalty and no global cap, exactly like every other experiment-history leg. A run whose history carries no running experiment and no recorded fold behaves byte-identically to today (every tactical candidate reads `slot_available: pass`).
- **Analysis-only.** `live_program` is an internal annotation carried forward like the Step 1f and displacement annotations; it is never written to disk and never rendered (see SKILL.md `Deliverable Purity Constraint`).

The `live_program` annotation reconciles against the **tactical** lane only (page-element candidates, where surface + lever occupancy is well defined). Strategic-lane cross-deliverable reconciliation is governed elsewhere (`detect-strategic.md` Step 3) and is not this read's concern.

**Output (added):** the `live_program` annotation (`program_state_readable`, `occupied_slots`, `recorded_folds`) carried forward to `phases/validate.md` Gate 7 (`slot_available`).

### Step 1h: Testing-Method Detection

Set a single run-level `testing_method` value, one of `ab | pre_post | cohort | operational`. It classifies how this engagement measures its experiments, not a per-hypothesis field. It is consumed downstream by construct (Step 5b feasibility branch and the feasibility render), validate (the tactical `powerable` / `segmentation_satisfied` gate reads), and score (Infeasible Routing and the traffic-volume Impact modifiers). Detection reuses inputs this phase already reads; it adds no new artifact dependency and no web or disk I/O.

**Resolution precedence (first match wins):**

1. **Explicit signal.** If the engagement-constraints input (the Step 1d source) carries an explicit `testing_method`, use it. An unknown or unmapped value falls through to `ab` with a one-line note that the value was not recognized.
2. **Experiment-history inference.** Else, if the experiment-history input is bound (the Step 1g source), infer from the `design_type` distribution across the in-scope records. Classify each record's `design_type` as A/B (`ab`, `randomized_ab`) or non-A/B (`pre_post`, cohort, operational); an unrecognized token outside those sets is excluded from the tally and does not shift the method off `ab` (mirroring tier 1's unknown-value fallback). If a majority (> 50%) of the classified in-scope records are non-A/B, set a non-A/B method (use `pre_post` as the representative token; the `construct.md` Step 5b feasibility branch treats all non-A/B designs identically, so the specific non-A/B token does not change feasibility). Otherwise (a non-A/B share at or below 50%, or no non-A/B records) set `ab`. A genuinely mixed engagement that runs both methods in regular use should set the explicit tier-1 signal rather than rely on this inference.
3. **Default.** Else `ab`.

Both reads are net-new to the skill: no shipped file reads `design_type` today, and the engagement-constraints `testing_method` read is likewise new (Step 1d reads delivery and governance state; Step 1g reads only `next_experiments` / `surfaces` / `priority` / `outcome`).

**Penalty-free and tri-state.** When neither signal is present the value is `ab`, with no Confidence effect and no behavior change from today. The `ab` default is the deliberate no-regression choice: an existing A/B engagement with no explicit signal keeps every feasibility computation, hard gate, and routing it has today.

**Known boundary (documented, not a defect to fix here).** No shipped producer writes an engagement-constraints `testing_method` field yet, so the explicit-signal branch is satisfied only once such a producer exists (the follow-up `engagement-context-testing-method-signal`). Until then a pre/post engagement is detected only through the experiment-history `design_type` inference. A pre/post client with neither an explicit signal nor experiment history therefore falls to the `ab` default: the one case this read-side change cannot cover, because the fix needs a write-side producer on a different resource. A history-less pre/post client should set the explicit signal once the producer ships.

**Output:** the run-level `testing_method` value, carried forward to construct (Step 5b), validate (tactical gate reads), and score (routing and Impact modifiers). Internal only; never rendered in the deliverable (see SKILL.md `Deliverable Purity Constraint`), which renders feasibility in natural language.
