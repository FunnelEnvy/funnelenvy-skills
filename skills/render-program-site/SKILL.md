...
`decisions` slots default empty and MUST be authored by this pass: **distill** the source
`## Sequencing` / decisions sections into the `.tl` / `.dec-grid` structures the comps use --
never paste a raw section as one blob. For `sequence`, emit a short scannable timeline of
`.tl-row` waves (target 3 to 6), each shaped `<div class="tl-row"><div class="tl-when"><span>When</span></div><div class="tl-body"><h4>Wave title</h4><p>a sentence or two</p></div></div>`;
use client-facing wave labels ("First", "After tracking is restored"), never internal experiment
numbers, and keep each row to a sentence or two. For `decisions`, emit the `.dec-col` columns the
comps use. An uncurated render leaves these slots showing their `[no source prose ...]` placeholder,
which reads as unfinished rather than dumping the raw section.
