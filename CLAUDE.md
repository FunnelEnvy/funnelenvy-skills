...
│   ├── browser-mode.md           # Canonical browser-mode detection contract (experiment-mockup, live-capture)
...
│   ├── kb-mode.md                # Canonical KB-mode dual-mode contract (all dual-mode skills; the module's own table is the registration source)
...
│   ├── prose-craft.md            # Humanizer / anti-AI-writing rules for agent-authored prose (render-program-site, hypothesis-generator, landing-page-generator, cro-roadmap-red-team)
...
│   ├── ga4-audit/
│   │   ├── SKILL.md              # GA4 analytics audit (single agent; direct API preferred, analytics-mcp fallback)
│   │   └── ga4_client.py         # Direct GA4 Data/Admin API query script
│   ├── aa-audit/
...
Skill CHANGELOG.md files and per-skill dev artifacts are omitted from the tree for brevity (render-program-site and experiment-mockup keep their changelogs inline in SKILL.md instead of a standalone file; registry_check validates both forms). The tree carries no version pins; versions live in SKILL.md frontmatter (source of truth), mirrored only in the Available Skills headers below, README.md, and each changelog -- all enforced by `scripts/registry_check.py` in CI.
...
- **Exception:** cro-roadmap-red-team reads the produced roadmaps (plus their cited sources) and writes a standalone critique with net-new analysis into the deliverables tree (`.claude/deliverables/roadmap-red-team.md`; KB mode `{kb_root}/deliverables/{scope}-roadmap-red-team.md`). Same class of exception as hypothesis-generator: new analysis, not synthesis, so not L2. Read-only toward its target (never edits the roadmap), no web research, no `.claude/context/` writes.
...
| `.claude/context/performance-profile.md` | L1 | ga4-audit (GA4 properties) or aa-audit (Adobe Analytics properties) | hypothesis-generator (ICE calibration + performance-driven hypotheses), live-capture (page selection) |
| `.claude/context/brand-voice.md` | L1 | voice-inference | positioning-framework Agent 3 (voice baseline), landing-page-generator (voice calibration), hypothesis-generator (voice-aware hypothesis framing) |
...
| `.claude/deliverables/experiments/<slug>/mockup-screenshot.png` | Browser screenshot of injected state (live and playwright modes) | experiment-mockup |
| `.claude/deliverables/experiments/<slug>/control-screenshot.png` | Browser screenshot of the unmodified control state, same scroll position and viewport (live and playwright modes; consumed by render-program-site for Before/After rendering) | experiment-mockup |
| `.claude/deliverables/program-site/` | Two-altitude program site (hub + per-bet and per-test spokes; KB mode: `{kb_root}/deliverables/{scope}-program-site/`) | render-program-site |
| `.claude/deliverables/roadmap-red-team.md` | Standalone dated roadmap critique (KB mode: `{kb_root}/deliverables/{scope}-roadmap-red-team.md`) | cro-roadmap-red-team |
...
13. **`/cro-roadmap-red-team [--scope <slug>] [--altitude both]`** (optional, independent red team of the produced roadmaps at both altitudes; read-only, produces a standalone critique with per-item dispositions and a research backlog. Run after hypothesis-generator, before committing the roadmap to a client)
...
### live-capture (v0.2.2, in active development)
...
- Phase 3 (Capture, live and playwright): Screenshot control + injected viewport, extract section HTML, build standalone mockup.html
- Phase 4 (Annotate, all modes): Write placement.md with CRO rationale, attention strategy, implementation notes
...
**Outputs (dual-mode):** legacy `.claude/deliverables/experiments/<slug>/`; KB mode `{kb_root}/deliverables/experiments/<slug>/` (mockup.html, placement.md, mockup-screenshot.png, control-screenshot.png)

**Runtime:** ~40-80K tokens (live, variable with iteration), ~30-50K tokens (static).

### render-program-site (v0.8.0)
...
**Phases:** (1) resolve three inputs + mode (HARD STOP on a missing sidecar); (2) run `render_site.py` to validate the gate and emit all structure/chrome with labeled prose slots; (3) LLM curation pass mapping gold body sections -> prose slots (drop-list applied); (4) humanizer pass over authored prose; (5) completion message (mode, bet/test counts, per-test mockup status, gate result, deferred-hosting note; the site itself is written by `render_site.py` in Phase 2 and prose-edited in 3-4).
