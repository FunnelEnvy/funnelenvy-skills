...
|-------|---------|-------------|
| [positioning-framework](skills/positioning-framework/SKILL.md) | 1.2.0 | Autonomous positioning and messaging framework from web research. Dual-mode output: legacy `.claude/context/` files, or KB-native artifacts when a CRO knowledge base binding is detected. In KB mode, auto-invokes render-default-deliverables to render gold deliverable artifacts |
| [ga4-audit](skills/ga4-audit/SKILL.md) | 2.5.0 | GA4 analytics audit with page grouping, opportunity sizing, element interactions, trend analysis, and AI-referrer (LLM) traffic segmentation. Dual-mode output: legacy `.claude/context/performance-profile.md`, or a typed KB-native `silver-performance-analysis` artifact when a CRO knowledge base binding is detected |
| [aa-audit](skills/aa-audit/SKILL.md) | 1.2.0 | Adobe Analytics audit producing the same performance-profile schema family as ga4-audit (stamped at the schema version it fully satisfies). Dual-mode output: legacy `.claude/context/performance-profile.md`, or a typed KB-native `silver-performance-analysis` artifact when a CRO knowledge base binding is detected |
| [hypothesis-generator](skills/hypothesis-generator/SKILL.md) | 1.17.0 | CRO experiment engine with 32 patterns, ICE scoring, test feasibility, contrarian filtering, and LIFT sequencing. When business-level levers qualify, also produces a separate strategic roadmap deliverable (program/offer/motion/asset-level experiments with non-A/B measurement designs like holdout, pre/post, and operational tracking, each with its own scoring and exclusions) alongside the tactical page-element roadmap. Dual-mode I/O: legacy `.claude/context/` files, or KB-native (reads the full scope silver set, writes a typed gold experiment roadmap) when a CRO knowledge base binding is detected. In KB mode with a completed-experiment knowledge base bound, generates net-new iteration experiments from prior test outcomes and reconciles tactical candidates against the live experiment program, folding any test a running experiment already owns into "What's Not Here" as an iteration inside it rather than a standalone duplicate |
| [landing-page-generator](skills/landing-page-generator/SKILL.md) | 2.0.3 | B2B paid landing page generator with brief, copy, design, and QA phases |
...
/cro-roadmap-red-team --scope b2c
```
